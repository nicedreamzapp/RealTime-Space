...
final class AudioManager: ObservableObject {
    ...
    private init() {
        audioFormat = engine.mainMixerNode.outputFormat(forBus: 0)
    ...
    public func playSFX(named name: String, at position: SIMD3<Float>, gain: Float = 1.0) {
        DispatchQueue.main.async {
            // Set sfxPlayer position for spatial audio
            self.sfxPlayer.position = AVAudio3DPoint(x: position.x, y: position.y, z: position.z)
